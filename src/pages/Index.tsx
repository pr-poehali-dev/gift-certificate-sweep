...
const Index = () => {
  ...
  const handleNextStep = () => {
    if (step === 1 && isValidNominal) setStep(2);
    else if (step === 2 && recipientName.trim()) setStep(3);
  };
    ...
    <div className="min-h-screen bg-background">
      <header className="border-b border-border/50">
        <div className="max-w-6xl mx-auto px-6 py-5 flex items-center justify-between">
          ...
          <a href="/admin" className="text-sm text-muted-foreground hover:text-foreground transition-colors font-body">
            Для ресторана
          </a>
        </div>
      </header>
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 pb-20">
          <div className="space-y-10 opacity-0 animate-slide-up" style={{ animationDelay: "0.2s" }}>
            <div className="flex items-center gap-4 text-sm font-body text-muted-foreground">
              <button onClick={() => setStep(1)} className={`flex items-center gap-2 transition-colors ${step >= 1 ? "text-foreground" : ""}`}>
              ...
              <button onClick={() => isValidNominal && setStep(2)} className={`flex items-center gap-2 transition-colors ${step >= 2 ? "text-foreground" : ""}`}>
            ...
            {step === 1 && (
              <div className="space-y-6">
                  ...
                  {NOMINALS.map((nominal) => (
                    <button
                      ...
                      className={`group relative py-5 px-4 border rounded-lg text-center transition-all duration-200 hover:border-foreground ${
                        selectedNominal === nominal ? "border-foreground bg-foreground text-background" : "border-border hover:shadow-sm"
                      }`}
                    >
                ...
                <Button onClick={handleNextStep} disabled={!isValidNominal} className="w-full h-14 text-base font-body font-medium rounded-lg bg-foreground text-background hover:bg-foreground/90 disabled:opacity-30">
                  Продолжить
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <Input type="text" placeholder="Александр" value={recipientName} onChange={(e) => setRecipientName(e.target.value)} className="h-14 text-lg font-display pl-4 border-border focus:border-foreground rounded-lg" />
                  </div>
                  <div>
                    ...
                    <Input type="text" placeholder="Мария" value={senderName} onChange={(e) => setSenderName(e.target.value)} className="h-14 text-lg font-display pl-4 border-border focus:border-foreground rounded-lg" />
                  </div>
                </div>
                <div className="flex gap-3">
                  <Button variant="outline" onClick={() => setStep(1)} className="h-14 px-6 rounded-lg border-border font-body">
                    <Icon name="ArrowLeft" size={18} />
                  </Button>
                  <Button onClick={handleNextStep} disabled={!recipientName.trim()} className="flex-1 h-14 text-base font-body font-medium rounded-lg bg-foreground text-background hover:bg-foreground/90 disabled:opacity-30">
                    Продолжить
                    <Icon name="ArrowRight" size={18} className="ml-2" />
                  </Button>
                </div>
            ...
            {step === 3 && (
              <div className="space-y-6">
                ...
                <div className="flex gap-3">
                  ...
                  <Button onClick={handlePurchase} disabled={isProcessing} className="flex-1 h-14 text-base font-body font-medium rounded-lg bg-foreground text-background hover:bg-foreground/90">
                    {isProcessing ? (
                      <>
          ...
          <div className="hidden lg:flex items-start justify-center pt-4 opacity-0 animate-fade-in" style={{ animationDelay: "0.4s" }}>
            <CertificatePreview nominal={currentNominal} recipientName={recipientName || "Имя получателя"} senderName={senderName} />
          </div>
        </div>
      </main>
